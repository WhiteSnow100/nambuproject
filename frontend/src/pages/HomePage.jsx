...
const HomePage = () => {
  ...
  const [triggerNavigation, setTriggerNavigation] = useState(false); // 추가 상태
  ...
  // word 상태가 업데이트된 후 handleButtonClick("word") 실행
  useEffect(() => {
    if (triggerNavigation) {
      setGptClick(1);
      // handleButtonClick("word");
      setTriggerNavigation(false); // 다시 초기화하여 중복 실행 방지
    }
  }, [triggerNavigation]); // triggerNavigation 상태를 의존성으로 설정
